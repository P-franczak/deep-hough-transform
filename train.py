...
def train(train_loader, model, optimizer, epoch, writer, args):
    ...
    for i, data in enumerate(bar):
        ...
        hough_space_loss = torch.nn.functional.binary_cross_entropy_with_logits(
            keypoint_map, hough_space_label
        )

        writer.add_scalar(
            "train/hough_space_loss", hough_space_loss.item(), epoch * iter_num + i
        )
        ...
        if i % CONFIGS["TRAIN"]["PRINT_FREQ"] == 0:
            visualize_save_path = os.path.join(
                CONFIGS["MISC"]["TMP"], "visualize", str(epoch)
            )
    ...
    print("total_loss_hough: {}".format(total_loss_hough))
...
def validate(val_loader, model, epoch, writer, args):
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(bar):
            ...
            writer.add_scalar(
                "val/hough_space_loss", hough_space_loss.item(), epoch * iter_num + i
            )
            ...
            binary_kmap = (
                key_points.squeeze().cpu().numpy() > CONFIGS["MODEL"]["THRESHOLD"]
            )
        ...
        logger.info(
            "Validation result: ==== Precision: %.5f, Recall: %.5f"
        ...
        if CONFIGS["MODEL"]["EDGE_ALIGN"]:
            total_recall_align = total_tp_align / (
                total_tp_align + total_fn_align + 1e-8
            )
            total_precision_align = total_tp_align / (
                total_tp_align + total_fp_align + 1e-8
            )
            f_align = (
                2
                * total_recall_align
            ...
            writer.add_scalar(
                "val/total_precison_align", total_precision_align.mean(), epoch
            )
            writer.add_scalar(
                "val/total_recall_align", total_recall_align.mean(), epoch
            )
            logger.info(
            ...
            acc = f_align.mean()
            logger.info(
                "Validation result (Aligned): ==== F-measure: %.5f" % acc.mean()
            )
            logger.info(
                "Validation result (Aligned): ==== F-measure@0.95: %.5f"
                % f_align[95 - 1]
            )
            writer.add_scalar("val/f-measure", acc.mean(), epoch)
...
def save_checkpoint(state, is_best, path, filename="checkpoint.pth.tar"):
    ...
    if is_best:
        shutil.copyfile(
            os.path.join(path, filename), os.path.join(path, "model_best.pth")
        )
...
if __name__ == "__main__":
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    logger.info(f"Using device: {device} line:50")
    main()

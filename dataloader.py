...
class SemanLineDataset(Dataset):
    ...
    def __init__(
        self, root_dir, label_file, split="train", transform=None, t_transform=None
    ):
    ...
    def collate_fn(self, batch):
        ...
        hough_space_label8 = torch.stack(
            [hough_space_label for hough_space_label in hough_space_label8]
        )
...
class SemanLineDatasetTest(Dataset):
    ...
    def __init__(self, root_dir, label_file, transform=None, t_transform=None):
        lines = [line.rstrip("\n") for line in open(label_file)]
        self.image_path = [join(root_dir, i + ".jpg") for i in lines]  # for TUSimple
...
def get_loader(
    ...
    if test is False:
        ...
        dataset = SemanLineDataset(
            root_dir, label_file, transform=transform, t_transform=None, split=split
        )
    else:
        transform = transforms.Compose(
            [
        ...
        dataset = SemanLineDatasetTest(
            root_dir, label_file, transform=transform, t_transform=None
        )
    if test is False:

...
class LineAnnotation(object):
    ...
    def oriental_mask(self):
        if self.__oriental_mask is None:
            ...
            for idx, l in enumerate(self.lines):
                mask1 = mask == (idx + 1)
                orient = (
                    round((l.angle() + np.pi / 2) / (np.pi / self.divisions))
                    % self.divisions
                )  # 0, 1, ..., 11
    ...
    def regression_label(self):
        if self.__regression_label is None:
            ...
            for idx, l in enumerate(self.lines):
                ...
                orient[idx] = l.angle()
            _, [indicesY, indicesX] = distance_transform_edt(
            ...
            angle[:] = [
                -np.pi / 2 + k * np.pi / self.divisions for k in range(self.divisions)
            ]
    ...
    def offset(self):
        if self.__offset is None:
            ...
            H, W = mask.shape
            bw_dist, bw_idx = distance_transform_edt(
    ...
    def rescale(self, r):
        ...
        self.size = (np.array(self.size) / r).astype(int).tolist()
